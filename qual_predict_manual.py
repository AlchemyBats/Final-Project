#Regression analysis on manual scores
...
#print("Loading manual keyword summary data...")
summary = pd.read_csv("manual_summaries/results.csv")
...
for _, row in summary.iterrows():
    ...
    # build same structure as LLM version, only sentiment source differs
...
# ---------------------------------------------------------
# Build keyword sentiment exactly once sentiment value
# ---------------------------------------------------------
df["total"] = df["positives"] + df["negatives"]
df = df[df["total"] > 0]

df["avg_sentiment"] = (df["positives"] - df["negatives"]) / df["total"]
#df["total"] = df["positives"] + df["negatives"]
#df = df[df["total"] > 0]

#df["avg_sentiment"] = (df["positives"] - df["negatives"]) / df["total"]
# ---------------------------------------------------------
# Train/Test Split (80/20) — same as LLM version
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Regression Model — same as LLM version
# ---------------------------------------------------------
...
# Predict future values (the test 20%)
test = test.copy()                     # <-- prevents SettingWithCopyWarning
...
plt.show()
# ---------------------------------------------------------
# Accuracy Metrics — same as LLM version
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Plot Actual vs Predicted — same as LLM version
# ---------------------------------------------------------
...
plt.title("Manual -- Actual vs Predicted Stock Price Change")
plt.xlabel("Date")
